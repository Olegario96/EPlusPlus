...
class FileManager(object):
	...
	def getHeaderFromSample(self, pathToFolder):
		try:
			csvFile = open(pathToFolder + "/sample.csv", 'r')
			csvReader = csv.reader(csvFile, delimiter=',')
			header = csvReader.__next__()
			...
			raise e("No sample found!")

	def getRowsFromCsvSample(self, pathToFolder):
		with open(pathToFolder + "/sample.csv", 'r') as csvFile:
			rows = []
			csvReader = csv.reader(csvFile, delimiter=',')
			csvReader.__next__()
			for row in csvReader:
	...
	##             was generated by the simulation and remove it. Also, this
	##             method removes the csv file named "sample"
	##
	## @param      self          Non static method
	...
	def removeDirectories(self, pathToFolder):
		...
		os.remove(pathToFolder + "/sample.csv")